...
class Administrator extends CI_Controller {
	...
	function publish_listberita(){
		...
		if (isset($_POST['submit1'])){
			$this->model_berita->list_berita_publish();
			redirect('administrator/publish_listberita/'.$this->input->post('id'));
		}elseif (isset($_POST['submit2'])){
			$this->model_berita->list_berita_revisi();
			redirect('administrator/publish_listberita/'.$this->input->post('id'));
		}else{
			$data['tag'] = $this->model_berita->tag_berita();
			$data['rows'] = $this->model_berita->list_berita_detail($id)->row_array();
			$this->template->load('administrator/template','administrator/mod_berita/view_berita_detail',$data);
		}
	}

	function delete_beritacatatan(){
		$id = $this->uri->segment(3);
		$this->model_berita->list_beritacatatan_delete($id);
		redirect('administrator/publish_listberita/'.$this->uri->segment(4));
	}
	...
	function iklanhome(){
		cek_session_admin();
		$data['record'] = $this->model_iklan->iklan_tengahadmin();
	...
	function iklansidebar(){
		cek_session_admin();
		$data['record'] = $this->model_iklan->iklan_sidebaradmin();
	...
	function delete_pesanmasuk(){
        ...
        $this->model_main->delete('hubungi',$id);
	...
	function manajemenuser(){
		cek_session_admin();
		$data['record'] = $this->model_users->users($this->uri->segment(3));
	...
	function detail_manajemenuser(){
		cek_session_admin();
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
			$this->model_users->users_bayarkan();
			redirect('administrator/detail_manajemenuser/'.$this->input->post('id'));
		}else{
			$data['rows'] = $this->model_users->users_edit($id)->row_array();
			$data['publish'] = $this->model_users->penghasilan_publish($id);
			$data['bayar'] = $this->model_users->penghasilan_bayar_history($id);

			$data['totpublish'] = $this->model_users->penghasilan_publish_total($id)->row_array();
			$data['totbayar'] = $this->model_users->penghasilan_bayar($id)->row_array();
			$data['koleksifoto'] = $this->model_users->koleksifoto($id, 0, 90);
			$data['semuateman'] = $this->model_users->semuateman($id);
			$data['permintaanteman'] = $this->model_users->permintaanteman($id);
			$data['unpublished'] = $this->model_berita->unpublished_berita($id);
			$data['published'] = $this->model_berita->published_berita($id);
			$this->template->load('administrator/template','administrator/mod_users/view_users_detail',$data);
		}
	}
	...
	function delete_koleksifoto(){
		cek_session_admin();
		$id = $this->uri->segment(3);
		$this->model_users->koleksifoto_delete($id,$this->uri->segment(3));
		redirect('administrator/detail_manajemenuser'.$this->uri->segment(3));
	}
